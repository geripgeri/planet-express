...
Amendment (2026-09-25): application backups use a separate `k8s-backup` bucket and `authentik-backup` Garage key. The `tofu-state` key remains limited to the state bucket. The dedicated key is managed by the `garage/k8s-backup` Terragrunt unit, copied into SOPS after apply, and injected into the CNPG backup Secret by the bootstrap script. This preserves least-privilege access between state management and application backup workloads.
